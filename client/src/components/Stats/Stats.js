...
class Stats extends Component {
  ...
  loadMapData(map) {
    const recordCities = _.map(this.state.data.recordCities, (entry) => {
      const data = entry;
      data.circleRadius = Math.sqrt(entry.recordFrac);
      data.circleColor = entry.recordTemp;
      return data;
    });
    const recordTemps = _.map(this.state.data.recordCities, entry => entry.recordTemp);
    const recordFracs = _.map(this.state.data.recordCities, entry => entry.recordFrac);
    return () => {
      map.addLayer({
        id: 'records',
        type: 'circle',
