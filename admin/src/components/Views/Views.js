...
class View extends Component {
  setBonus(event) {
    const bonus = this.bonusBox.value;
    const id = this.props.view.id;
...
photo page
              ...
              <button className="btn border">
                <a href={view.owner.page} target="_blank">
  ...
  render() {
    return (
      <div className="view-list">
        {_.map(this.props.views, view => (<View view={view} key={view.id} />))
                }
      </div>
    );
  }
...
class Views extends Component {
    ...
    return Promise.all([fetch('/api/views'), fetch('/api/cities')])
      ...
      .then((results) => {
        ...
        for (const view of views) {
          view.city = _.find(cities, entry => entry.geonameid === view.geonameid);
        }
  ...
  render() {
    const filteredViews = _.filter(this.state.views, view => view.city.name.toLowerCase()
